...
def apply_grid_layout(grid):
    ...
    grid.layout.overflow_x = "auto"
    grid.layout.overflow_y = "visible"
...
WIDGET_OUTPUT_BLOCK = r'''# Capture stdout, stderr, rich displays, figures, warnings, and tracebacks from widget callbacks.
...
def _replace_welcome_assignments(source: str) -> str:
    """Replace known Welcome table cells with the wrapping helper."""
    replacements = {
...
def _upgrade_welcome_source(source: str, relative_path: Path) -> str:
    ...
    # Make the Open Notebook control shrink with the final column while keeping
    # enough width for a readable single-line button.
    ...
    # Preserve callback output from subprocesses, rich displays and tracebacks.
    normalized = normalized.replace(
    ...
    normalized = normalized.replace("apply_grid_column_widths(grid)", "apply_grid_layout(grid)")

    # Canonicalize the CSS block without touching any repository-specific data.
...
def update_welcome_notebook(repo_root: Path, relative_path: Path) -> bool:
    ...
    print(f"Updated wrapping Welcome table and widget output in {relative_path}")
    return True
